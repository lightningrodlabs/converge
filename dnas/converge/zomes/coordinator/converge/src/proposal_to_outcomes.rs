...
pub fn add_outcome_for_proposal(input: AddOutcomeForProposalInput) -> ExternResult<()> {
    create_link(input.base_proposal_hash.clone(), input.target_outcome_hash.clone(), LinkTypes::ProposalToOutcomes, ())?;
...
pub fn get_outcomes_for_proposal(proposal_hash: ActionHash) -> ExternResult<Vec<Link>> {
    get_links(
        GetLinksInputBuilder::try_new(proposal_hash, LinkTypes::ProposalToOutcomes)?.build(),
    )
}
