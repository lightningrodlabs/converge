...
onMount(async () => {
  if (proposalHash === undefined) {
    throw new Error(`The proposalHash input is required for the CriteriaForProposal element`);
  }
  ...
  try {
    const records = await client.callZome({
      ...
      role_name: 'converge',
      zome_name: 'converge',
      fn_name: 'get_criteria_for_proposal',
  ...
  client.on('signal', signal => {
    ...
    if (payload.link_type !== 'ProposalToCriteria') return;
...
<span>No criteria found for this proposal.</span>
{:else}
