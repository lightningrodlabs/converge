...
onMount(async () => {
  if (supporter === undefined) {
    throw new Error(`The supporter input is required for the CriteriaForSupporter element`);
  }
  ...
  client.on('signal', signal => {
    ...
    if (payload.link_type !== 'SupporterToCriteria') return;
...
<span>No criteria found for this supporter.</span>
{:else}
