...
import { getCellIdFromRoleName, joinHrlString, splitHrlString, } from "@holochain-open-dev/utils";
...
export const searchAgentPlugin = new Plugin({
    key: pluginKey,
    state: {
        init() {
            return "hidden";
    ...
    props: {
        ...
        handleTextInput(view, from, to, text) {
            ...
            if (state && state !== "hidden") {
                setTimeout(() => {
                    state.dropdownEl.searchFilter = view.state.doc.textBetween(state.mentionCharIndex + 1, to + 1);
                ...
                const dropdownEl = document.createElement("search-agent-dropdown");
                ...
                dropdownEl.addEventListener("agent-selected", (e) => {
                    const agentPubKey = e.detail.agentPubKey;
                    const state = this.getState(view.state);
                    if (!state || state === "hidden")
                        return;
                    const tr = view.state.tr;
                    tr.replaceRangeWith(state.mentionCharIndex, state.lastCharIndex + 1, agentType.create({
                        agentPubKey,
                    }));
                    view.dom.getRootNode().removeChild(dropdownEl);
                    view.dispatch(tr.setMeta(pluginKey, "hidden"));
                });
                dropdownEl.addEventListener("sl-hide", () => setTimeout(() => {
                    view.dom.getRootNode().removeChild(dropdownEl);
...
let TextareaWithMentions = class TextareaWithMentions extends SlTextareaProsemirror {
    constructor() {
        super(...arguments);
        this.helpText = msg("Press '@' to mention an agent.");
    }
    async firstUpdated() {
        super.firstUpdated();
    ...
    get value() {
        var _a;
        if (!((_a = this.input) === null || _a === void 0 ? void 0 : _a.quill))
            return "";
        const contents = this.input.quill.getContents();
        const array = contents.ops.map((delta) => {
...
__decorate([
    ...
    property()
], TextareaWithMentions.prototype, "store", void 0);
__decorate([
    property()
], TextareaWithMentions.prototype, "helpText", void 0);
TextareaWithMentions = __decorate([
    localized(),
    customElement("textarea-with-mentions")
], TextareaWithMentions);
export { TextareaWithMentions };
//# sourceMappingURL=textarea-with-mentions.js.map