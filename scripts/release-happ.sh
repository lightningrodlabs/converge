...
#   1. `workdir/converge.happ` from a local `npm run build:happ:release` — used when
...
if [ $# -ge 1 ]; then
  ...
  happ="$tmp/wa/converge.happ"
else
  happ="workdir/converge.happ"
  if [ ! -f "$happ" ]; then
...
notes="Frozen canonical converge.happ reused by every webhapp release to keep all installs on the same network. DNA sha256: $EXPECTED_SHA. Do NOT rebuild."
...
echo "Done. release-webhapp.yaml will download converge.happ from $HAPP_TAG."
